...
export const COMPACT_REQUEST_ROW_HEIGHT = 84
  ...
  if (compact) {
    return (
      <div
        onClick={onClick}
        className={cn(
          'flex h-full min-h-0 min-w-0 cursor-pointer items-center gap-2.5 overflow-hidden border-b border-border/50 px-2.5 py-3 text-sm transition-colors last:border-0',
          isSelected
      ...
      >
        <div className="flex w-0 flex-1 flex-col justify-center gap-2 self-stretch">
          ...
          <span className="line-clamp-2 min-h-[2.5rem] pr-1 text-[12px] leading-5 text-foreground">
            {compactTitle}
          </span>
        </div>

        <span
          className="w-[56px] shrink-0 self-stretch text-right text-[10px] tabular-nums leading-4 text-muted-foreground"
        >
          <span className="block pt-1">{formatDateOnly(request.time)}</span>
