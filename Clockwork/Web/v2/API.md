...
| GET | `/__clockwork/failures?cursor=&type=&status=&search=&since=&limit=` | cursor-paged failed-events list → `{ failures, nextCursor }` (`nextCursor` = request id to resume the backward walk from, or `null` when exhausted; `limit` clamped 1..100) | 失败事件 (RequestList failed mode) |
...
| GET | `/__clockwork/operations/{category}?since=&until=&type=&limit=&offset=` | **operations center**: per-category (`database\|cache\|redis\|log\|events\|views\|notifications`) KPIs + a flattened list of individual operations, each carrying `requestId`/`requestUri`/`requestType`/`requestTime` for reverse-lookup. KPIs and `total` cover the whole matched window (not paged); the `operations` list is offset-paged (`limit` + `offset`, ordered `time DESC, request_id DESC, seq DESC` for stable paging), and `type` is filtered server-side. `notifications` also merges the legacy `emailsData` (old Swift-based mail). Client-side search/sort apply over the loaded pages. | Operations center |
